 ...
 * @since 18/01/2017
 */
internal class PhotoEditorImplOriginal @SuppressLint("ClickableViewAccessibility") constructor(
    builder: PhotoEditorOriginal.Builder
) : PhotoEditorOriginal {
    private val photoEditorView: PhotoEditorViewOriginal = builder.photoEditorView
    private val viewState: PhotoEditorViewStateOriginal = PhotoEditorViewStateOriginal()
    private val imageView: ImageView = builder.imageView
    private val deleteView: View? = builder.deleteView
    private val drawingView: DrawingViewOriginal? = builder.drawingView
    private val mBrushDrawingStateListener: BrushDrawingStateListenerOriginal =
        BrushDrawingStateListenerOriginal(builder.photoEditorView, viewState)
    private val mBoxHelper: BoxHelperOriginal = BoxHelperOriginal(builder.photoEditorView, viewState)
    ...
    private val mGraphicManager: GraphicManagerOriginal = GraphicManagerOriginal(builder.photoEditorView, viewState)
     ...
     */
    private fun getMultiTouchListener(isPinchScalable: Boolean): MultiTouchListenerOriginal {
        return MultiTouchListenerOriginal(
            deleteView,
            photoEditorView,
            imageView,
            isPinchScalable,
    ...
    override suspend fun saveAsFile(
        ...
        val photoSaverTask = PhotoSaverTaskOriginal(photoEditorView, mBoxHelper, saveSettings)
    ...
    override fun saveAsFile(
        imagePath: String,
        saveSettings: SaveSettings,
        onSaveListener: PhotoEditorOriginal.OnSaveListener
    ) {
    ...
    @RequiresPermission(allOf = [Manifest.permission.WRITE_EXTERNAL_STORAGE])
    override fun saveAsFile(imagePath: String, onSaveListener: PhotoEditorOriginal.OnSaveListener) {
    ...
    init {
        ...
        val mDetector = GestureDetector(
            context,
            PhotoEditorImageViewListenerOriginal(
                viewState,
                object : PhotoEditorImageViewListenerOriginal.OnSingleTapUpCallback {
